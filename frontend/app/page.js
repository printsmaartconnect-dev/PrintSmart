...
function HomeContent() {
  ...
  // Detect QR scan on page load and redirect to take-a-print page to enforce the requested flow
  useEffect(() => {
    const shopId = searchParams.get('shopId')
    if (shopId) {
      router.push(`/take-a-print?shopId=${shopId}`)
    }
  }, [searchParams, router])
  ...
  return (
    <div className="wave-bg min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-12">
        ...
        {/* Center Card (default when no QR, or scanned QR is redirected to take-a-print) */}
        <button
