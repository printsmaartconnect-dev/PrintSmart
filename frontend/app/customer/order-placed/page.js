  ...
  if (!order?.orderId) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <p className="text-gray-600 font-semibold text-lg">No active order found.</p>
      </div>
    )
  }
  ...
  return (
    <div className="wave-bg min-h-screen flex flex-col items-center justify-center px-4 sm:px-6 lg:px-10 py-8 lg:py-10">
      ...
      <div className="mb-8 text-center">
        ...
        <h1 className="text-3xl font-bold text-gray-900 font-brand">Order Placed</h1>
        <p className="text-gray-600 mt-2 font-medium">Your print order has been successfully sent to the shop.</p>
      </div>
      ...
      <div className="glassmorphism w-full max-w-md sm:max-w-xl lg:max-w-4xl p-6 sm:p-8 lg:p-10">
        <div className="flex items-center justify-between mb-6">
          <BackButton />
          <span className="text-sm font-semibold text-green-600">Order Completed</span>
        </div>
        ...
        <p className="text-center text-gray-600 text-sm font-medium mb-8">
          You can track the progress of your printing order below.
        </p>
        ...
        <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 space-y-4 mb-8">
          ...
          <div className="flex justify-between items-center pb-3 border-b border-gray-200 text-sm">
            <span className="text-gray-600 font-semibold">Printing Location</span>
            <span className="font-bold text-gray-800">{order.shopName || 'Smart Print Station'}</span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-gray-600 font-semibold flex items-center gap-1"><Clock size={16} className="text-indigo-600" /> Estimated Wait Time</span>
            <span className="font-bold text-green-600 text-base">
              {order.estimatedTime ? `${order.estimatedTime} mins` : '2–7 mins'}
            </span>
          </div>
        </div>
