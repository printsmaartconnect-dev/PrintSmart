...
class AnalyticsService {
     ...
     */
    static async getBusinessMetrics(shopkeeperId) {
        ...
        const yesterdayOrders = await db_1.default.order.findMany({
            where: {
                shopkeeperId,
        ...
        const weeklyOrders = await db_1.default.order.findMany({
            where: {
                shopkeeperId,
        ...
        const pendingOrdersCount = await db_1.default.order.count({
            where: {
                shopkeeperId,
                status: "PENDING"
            }
        });
        const cancelledOrdersCount = await db_1.default.order.count({
            where: {
                shopkeeperId,
                status: "CANCELLED",
                createdAt: { gte: periods.startOfWeek }
        ...
        const lastWeekOrdersCount = await db_1.default.order.count({
            where: {
                shopkeeperId,
        ...
        const allMonthlyOrdersWithUsers = await db_1.default.order.findMany({
            where: {
                shopkeeperId,
                createdAt: { gte: periods.startOfMonth },
                status: { in: ["COMPLETED", "DOWNLOADED"] }
        ...
        const configs = await db_1.default.printConfiguration.findMany({
            where: {
                order: {
                    shopkeeperId,
                    createdAt: { gte: periods.startOfMonth },
        ...
        const ordersForHours = await db_1.default.order.findMany({
            where: {
                shopkeeperId,
                createdAt: { gte: periods.startOfMonth }
            },
        ...
        const peakHours = Object.entries(hoursMap)
            ...
            .slice(0, 3);
        // 8. Printer Usage
        const printerStats = await db_1.default.printerStatistics.findMany({
            where: { shopkeeperId }
        });
        const totalPagesPrintedWeekly = await db_1.default.printConfiguration.aggregate({
            where: {
                order: {
                    shopkeeperId,
                    createdAt: { gte: periods.startOfWeek }
