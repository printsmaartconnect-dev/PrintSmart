...
class RecommendationService {
    static loadPrinterRules() {
        try {
            const filePath = path_1.default.resolve(__dirname, "../../knowledge/printer_rules.json");
    ...
    static async getRecommendations(metrics) {
        const recommendations = [];
        // 1. Gather Pricing Advisor recommendations
        const pricingRecs = pricing_service_1.PricingService.getRecommendations(metrics);
        recommendations.push(...pricingRecs);
        // 2. Gather Inventory Advisor recommendations
        const inventoryRecs = inventory_service_1.InventoryService.getRecommendations(metrics);
        recommendations.push(...inventoryRecs);
        // 3. Printer Advisor recommendations
        const printerRules = this.loadPrinterRules();
        metrics.printerStatus.forEach((printer) => {
            // Rule: Low ink level
            const inkRule = printerRules.find((r) => r.id === "PTR-01");
            ...
            const maintRule = printerRules.find((r) => r.id === "PTR-02");
