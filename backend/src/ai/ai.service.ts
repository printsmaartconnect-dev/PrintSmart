...
import { SYSTEM_PROMPT_CHAT, SYSTEM_PROMPT_RECOMMENDATION } from "./prompts/systemPrompt";
...
export class AIService {
  private static async callGemini(
    ...
    if (!apiKey) {
      throw new Error(
        "GEMINI_API_KEY is not configured. Please add it to your backend .env file or supply it from the settings panel in the frontend."
      );
    }
  ...
  static async chat(
    ...
    const systemPrompt = SYSTEM_PROMPT_CHAT.replace(
      "{{BUSINESS_CONTEXT}}",
   ...
   */
  private static generateFallbackResponse(context: any, message: string): string {
...
Your top service volume is **${context.topServices[0].name}** with **${context.topServices[0].count} copies** printed this month. Keep customers engaged by maintaining printer paper feeds.`;
    ...
    if (query.includes("paper") || query.includes("inventory") || query.includes("stock")) {
      const lowItems = context.inventoryStatus.filter((i: any) => i.status === "LOW");
      if (lowItems.length > 0) {
        return `⚠️ **Critical Stock Alerts**:
${lowItems.map((i: any) => `- **${i.itemName}** is low: currently **${i.quantity} ${i.unit}** remaining.`).join("\n")}
